...
export default function Project({ image, title, desc, date, id }) {
  return (
    <div>
      <Link href={`/projects/details/${id}`} className='flex flex-col cursor-pointer group relative min-h-36 w-full overflow-hidden transition-all duration-200 items-center gap-2 group border p-5'>
        <div className="overflow-hidden relative w-full h-full ">
          <Image src={image} alt="project-image" height={500} width={500} className='object-cover group-hover:scale-110 transition-all duration-300 h-full w-full' />
