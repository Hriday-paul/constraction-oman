import { useAllClientsQuery, useAllSectorsQuery, useEditProjectMutation, usePeoplesQuery } from "@/Redux/Api/Api";
...
const EditProject = React.memo(({ project }) => {
    ...
    const [updateProject, { isLoading, isError, isSuccess, error }] = useEditProjectMutation();
    ...
    const {
        ...
        defaultValues: {
            ...
            sector: project?.project_type_id,
            startDate: project?.start_date,
            completeDate: project?.end_date,
            details: project?.details,
            pManager: project?.project_manager,
            isBest: project?.is_best_projects
        }
    });
    ...
    const handleEdit = async (data) => {
        ...
        await updateProject({ id: project?.id, formData: form }).unwrap();
    };
    ...
    useEffect(() => {
        ...
        if (isError) {
            toast.error(error?.data?.error || 'Project update failed, try again');
            if (error?.status == 401) {
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <input
                                            {...field}
                                            type="checkbox"
                                            defaultChecked={project?.is_best_projects}
                                            className="
